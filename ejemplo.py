...
class Ejemplo:
    ...
    def run(self):
        ...
        if result:
            ...
            # geo = GeoCoding(self.iface)
            # geo.config()
            x, y = geocode_address(address)
            # geocoder = geo.get_geocoder_instance()
            # result = geocoder.point_geodecode(address)
            
            c = canvas.Canvas("/home/tobares/Descargas/reporte3.pdf", pagesize=letter)
            message = "Dirección:   "+address+'\n'+ ' x: '+str(x)+' y: '+str(y)
            c.drawString(100, 750, message)
            c.save()
            self.iface.messageBar().pushMessage("Success", " Direccion: "+address+'  '+str(x)+'  '+str(y),level=Qgis.Success, duration=3)
