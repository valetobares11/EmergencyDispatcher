...
class RouteProvider(object):
    def __init__(self):
        ...
        self.__hereBaseURL_V8_Exclusion = 'https://router.hereapi.com/v8/routes?origin=%s&destination=%s&transportMode=%s&avoid[areas]=polygon:%s&return=polyline,actions,instructions,summary&apiKey=%s&lang=es&mode=fastest;%s;traffic:enabled;'
        self.__hereBaseURL_V8 = 'https://router.hereapi.com/v8/routes?origin=%s&destination=%s&transportMode=%s&return=polyline,actions,instructions,summary&apiKey=%s&lang=es&mode=fastest;%s;traffic:enabled;'
